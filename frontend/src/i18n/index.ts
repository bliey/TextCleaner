...
const zh: Dict = {
  ...
  // ---- 输出与执行 ----
  'out.title': '输出与执行',
  'out.hint': '所有文件均在本地处理，不会上传到服务器',
  'out.exportMode': '导出方式',
  'out.downloadZip': '导出为 ZIP',
  'out.downloadZipDesc': '打包为 TextCleaner_Output.zip（保持文件夹结构）',
  'out.downloadFiles': '逐个下载文件',
  'out.downloadFilesDesc': '逐个下载处理后的文件（浏览器可能会请求允许多个下载）',
  'out.exportFolder': '导出到文件夹',
  'out.exportFolderDesc': '多文件直接写入所选文件夹；单个文件弹出系统保存对话框',
  'out.privacy': '隐私保护：所有文件均在您的设备本地处理，不会上传到任何服务器。',
  'out.encoding': '输出编码',
  ...
  'settings.sponsor': '赞助',
  'settings.sponsorDesc': '如果这个工具对你有帮助，欢迎支持一下。点击支付方式查看二维码。',
  'settings.sponsorAlipay': '支付宝',
  'settings.sponsorWechat': '微信',
  'settings.sponsorPaypal': 'PayPal',
  'settings.save': '保存',
  'settings.close': '关闭',
  'settings.reset': '恢复默认',
...
const en: Dict = {
  ...
  'out.hint': 'All files are processed locally; nothing is uploaded.',
  'out.exportMode': 'Export mode',
  'out.downloadZip': 'Export as ZIP',
  'out.downloadZipDesc': 'Packages all results into TextCleaner_Output.zip (preserving folder structure)',
  'out.downloadFiles': 'Download files individually',
  'out.downloadFilesDesc': 'Downloads each processed file separately; the browser may ask permission for multiple downloads.',
  'out.exportFolder': 'Export to folder',
  'out.exportFolderDesc': 'Writes multiple files directly into a folder you choose; a single file uses the save dialog.',
  'out.privacy': 'Privacy: all files are processed locally on your device and are never uploaded.',
