...
const zh: Dict = {
  ...
  'out.exportMode': '导出方式',
  'out.downloadZip': '导出为 ZIP（默认）',
  'out.downloadZipDesc': '打包为 TextCleaner_Output.zip（保持文件夹结构）',
  'out.downloadFiles': '逐个导出文件',
  'out.downloadFilesDesc': '逐个导出处理后的文件；桌面端直接写入所选文件夹',
...
const en: Dict = {
  ...
  'out.downloadZip': 'Export as ZIP (default)',
  'out.downloadZipDesc': 'Packages all results into TextCleaner_Output.zip (preserving folder structure)',
  'out.downloadFiles': 'Export files individually',
  'out.downloadFilesDesc': 'Exports each processed file; on desktop they are written to a folder you choose.',
